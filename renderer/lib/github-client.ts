// Browser-safe GitHub client. Mirrors main/lib/github.ts — they must stay in sync
// (same pattern as renderer/lib/types.ts ↔ main/lib/types.ts). Only used by the
// web-dev fallback in renderer/lib/browser-api.ts; in Electron the renderer talks
// to GitHub via IPC and never imports this directly.
import type {
  CIStatus,
  PRState,
  PullRequest,
...
export async function fetchAuthoredPRs(
  ...
  const { data } = await githubGraphql<GqlSearchResponse>(token, PR_SEARCH_QUERY, {
    q,
    first: limit,
  })
